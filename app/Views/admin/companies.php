...
<link rel="stylesheet" href="<?=base_url()?>css/admin/perusahaan.css">
  ...
  <div class="box">
    ...
    <div class="row-2">
      ...
      <table>
        ...
        <tbody>
          <?php foreach ($perusahaan as $key => $perusahaanItem): ?>
            <tr>
              <td><?= $key + 1 ?></td>
              <td><?= $perusahaanItem['nama_perusahaan'] ?></td>
              <td><?= $perusahaanItem['jenis_perusahaan'] ?></td>
              <td><?= $perusahaanItem['alamat_perusahaan'] ?></td>
              <td><?= $perusahaanItem['contact'] ?></td>
              <td>
              <a href="<?= base_url('admin/hapusPerusahaan/' . $perusahaanItem['id']) ?>"onclick="return confirm('Apakah Anda yakin ingin menghapus pegawai ini?')"><button type="button" class="btn btn-danger">Delete</button></a>
                <button type="button" class="btn btn-success editButton" data-perusahaanid="<?= $perusahaanItem['id'] ?>"
                data-nama="<?= $perusahaanItem['nama_perusahaan'] ?>"
                  data-jenis="<?= $perusahaanItem['jenis_perusahaan'] ?>"
                  data-alamat="<?= $perusahaanItem['alamat_perusahaan'] ?>"
...
<div class="popup" id="addPopup">
  <div class="popup-content">
    ...
    <form id="addForm" action="<?= base_url('admin/addDataPerusahaan') ?>" method="post">
...
<div class="popup" id="editPopup">
  <div class="popup-content">
    ...
    <form id="editForm" action="<?= base_url('admin/updateDataPerusahaan') ?>" method="post">
      <input type="text" id="editCompanyId" name="id">
