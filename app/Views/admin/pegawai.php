...
<link rel="stylesheet" href="<?=base_url()?>css/admin/pegawai.css">
...
<div class="content">
      ...
      <table>
        ...
        <tbody>
          <?php foreach ($pegawai as $key => $pegawaiItem): ?>
            <tr>
              <td><?= $key + 1 ?></td>
              <td><?= $pegawaiItem['username'] ?></td>
              <td><?= $pegawaiItem['nama'] ?></td>
              <td><?= $pegawaiItem['struktur'] ?></td>
              <td>********</td>
              <td>
              <a href="<?= base_url('admin/hapusPegawai/' . $pegawaiItem['id']) ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus pegawai ini?')">
                <button type="button" class="btn btn-danger">Delete</button>
              </a>

                <button type="button" class="btn btn-success editButton" data-pegawaid="<?= $pegawaiItem['id'] ?>"
                  data-username="<?= $pegawaiItem['username'] ?>"
  ...
  <div class="popup" id="addPopup">
    <div class="popup-content">
      ...
      <form id="addForm" action="<?= base_url('admin/addDataPegawai') ?>" method="post">
  ...
  <div class="popup" id="editPopup">
    <div class="popup-content">
      ...
      <form id="editForm" action="<?= base_url('admin/updateDataPegawai') ?>" method="post">
        <input type="text" id="editPegawaiId" name="id">
